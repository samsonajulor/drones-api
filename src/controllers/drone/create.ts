...
async function create(req: Request, res: Response) {
  try {
    ...
    const newDrone = await DroneService.create({ serialNumber, model, weight: 0, battery, state: 'IDLE' });
    return apiResponse('create', res, RESPONSE.success, HttpStatusCode.OK, JSON.stringify(newDrone), 'drone created successfully');
  } catch (error) {
